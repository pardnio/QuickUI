...
// ! 1.*.* 移除 animation =
$document[_head][_appendChild](createElement("style", `
*[\\:effect="fade-in"],
*[effect="fade-in"] {
    opacity: 0;
...
function parseToHTML(result) {
    ...
    for (let e of result) {
        ...
        const tag = e[_tag] + (e[_id] == null ? "" : "#" + e[_id]) + (e[_class] == null ? "" : "." + e[_class][_replace](/\s/g, "."));
...
function removeEmptyTextNode(element) {
    return $Array[_from](element[_childNodes]).filter((e) => e[_nodeType] === $Node[_ELEMENT_NODE] ||
...
function setLazyloadListener() {
    ...
    lazyloadObserver = new $IntersectionObserver((entries, _) => {
        ...
        // 批量處理圖片載入
        Promise.all(toLoad.map(({ dom, src }) => check200(src, true)
            .then((res) => {
            dom[_src] = res[_src];
        })
            .catch((err) => {
...
class QUI {
    ...
    #patchDoCreate(targetNode, patch) {
        ...
        // * 創建新元素或文本節點
        const newElement = typeof patch[_vdom] === "string" ? $document[_createTextNode](patch[_vdom]) : this.#createElement(patch[_vdom]);
    ...
    #getNodeByPath(root, path) {
        ...
        let childNodes = $Array
            ...
            .filter((child) => child.nodeType === $Node[_ELEMENT_NODE] ||
...
class vDOM {
    ...
    constructor(element) {
        ...
        this[_children] = removeEmptyTextNode(element)[_map]((child) => child.nodeType === Node[_ELEMENT_NODE]
            ? // * 子元素非文字，循環嵌套
                new _a(child)
            : // * 子元素非文字，循環嵌套
        ...
        while (index < vdom[_children][_length]) {
            ...
            if ($Array[_isArray](data)) {
                ...
                for (const childIndex in data) {
                    ...
                    new_child[_data] = $JSON[_parse]($JSON[_stringify]({
        ...
        while (index < vdom[_children][_length]) {
            ...
            ((_) => {
                for (let node_index of ary) {
                    ...
                    // 取得目標節點的條件屬性值
                    const attr_value = node_target[_props][tagIf] || node_target[_props][tagElseIf] || node_target[_props][tagElif];
            ...
            if (typeof child == "string") {
                ...
                for (let item of child[_match](new $RegExp(regexText, "g")) || []) {
                    ...
                    if (fit[_startsWith]("i18n.")) {
                        ...
                        vdom[_children][index] = val[_replace](new $RegExp("{{\\s*?" + fit[_replace](/(\?|\.|\+|\*|\\|\%|\-|\(|\))/g, "\\$1") + "\\s*?}}", "i"), translatedValue);
                    }
                }
                for (let item of child[_match](new $RegExp(regexText, "g")) || []) {
                    const val = vdom[_children][index] || "";
                    ...
                    vdom[_children][index] = val[_replace](new $RegExp("{{\\s*?" + fit[_replace](/(\?|\.|\+|\*|\\|\%|\-|\(|\))/g, "\\$1") + "\\s*?}}", "i"), value);
                }
            }
            // * 子節點為 vDOM
            else if (child != null) {
